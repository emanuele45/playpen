		...
		function onDocReceived(XMLDoc)
		{
			...
			for (i = 0; i < numChanges; i++)
			{
				...
				else
				{
					...
					newItem.innerHTML = "' . JavaScriptEscape('
						<div class=\"content\">
							<div class=\"message_header\">
								<a class=\"split_icon float" + (curSection == "selected" ? "left" : "right") + "\" href=\"" + smf_prepareScriptUrl(smf_scripturl) + "action=splittopics;sa=selectTopics;subname=' . $context['topic']['subject'] . ';topic=' . $context['topic']['id'] . '.' . $context['not_selected']['start'] . ';start2=' . $context['selected']['start'] . ';move=" + (curSection == "selected" ? "up" : "down") + ";msg=" + curId + "\" onclick=\"return select(') . '" + (curSection == "selected" ? "up" : "down") + "' . JavaScriptEscape(', " + curId + ");\">
									<img src=\"' . $settings['images_url'] . '/split_" + (curSection == "selected" ? "de" : "") + "select.png\" alt=\"" + (curSection == "selected" ? "&lt;-" : "-&gt;") + "\" />
								</a>
								<strong>" + curChange.getElementsByTagName("subject")[0].firstChild.nodeValue + "</strong> ' . $txt['by'] . ' <strong>" + curChange.getElementsByTagName("poster")[0].firstChild.nodeValue + "</strong>
								<br /><em>" + curChange.getElementsByTagName("time")[0].firstChild.nodeValue + "</em>
							</div><div class=\"post\">" + curChange.getElementsByTagName("body")[0].firstChild.nodeValue + "</div>
						</div>') . '";
